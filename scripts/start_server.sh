...
sudo docker pull $SPRING_APP_IMAGE

# 4. docker-compose로 컨테이너 실행 (기존 컨테이너가 있다면 중지 후 재시작)
sudo -E docker compose -p $PROJECT_NAME --env-file "$ENV_FILE" down
sudo -E docker compose -p $PROJECT_NAME --env-file "$ENV_FILE" up -d
