...
internal class SearchOrchestratorTest : BehaviorSpec({
    ...
    Given("유효한 검색 키워드가 주어졌을 때") {
        ...
        every { mockDto.getTickerId() } returns UUID.randomUUID()
        every { mockDto.getTickerCode() } returns "AAPL"
        every { mockDto.getShortCompanyName() } returns "Apple"
        every { mockDto.getFullCompanyName() } returns "Apple Inc."
